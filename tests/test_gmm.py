...
def test_ooc():
	...
	gmm2 = GeneralMixtureModel.from_samples(MultivariateGaussianDistribution, 
		3, X, init='first-k', max_iterations=5, batch_size=500)
	...
	assert_almost_equal(gmm.log_probability(X).sum(), gmm3.log_probability(X).sum())
...
def test_minibatch():
	...
	assert_almost_equal(gmm3.log_probability(X).sum(), gmm4.log_probability(X).sum())
